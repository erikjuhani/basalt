...
/// Direction a find (`f`/`F`/`t`/`T`) scans.
...
/// Whether a text object spans its delimiters (`a`) or just the inside (`i`).
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
mod tests {
    ...
    fn find_char_forward_and_till() {
        ...
        assert_eq!(find_char(text, 0, 'x', Direction::Forward, true), Some(2)); // 't' stops before
        assert_eq!(find_char(text, 0, 'z', Direction::Forward, false), None);
    ...
    fn find_char_backward_and_till() {
        ...
        assert_eq!(find_char(text, 6, 'x', Direction::Backward, true), Some(4)); // 'T' stops after
        assert_eq!(find_char(text, 6, 'z', Direction::Backward, false), None);
    }
    ...
    fn nth_char_moves_and_clamps_on_the_line() {
        let text = "abc\ndef";
        assert_eq!(nth_char_right(text, 0, 2), 2); // a -> c
        assert_eq!(nth_char_right(text, 0, 9), 3); // clamps just past the last char
        assert_eq!(nth_char_left(text, 2, 1), 1); // c -> b
        assert_eq!(nth_char_left(text, 2, 9), 0); // clamps at the line start
    }

    #[test]
    fn nth_char_steps_over_multibyte() {
        let text = "aéb"; // é is two bytes
        assert_eq!(nth_char_right(text, 0, 1), 1); // onto 'é'
        assert_eq!(nth_char_right(text, 0, 2), 3); // past 'é' onto 'b'
        assert_eq!(nth_char_left(text, 3, 1), 1); // 'b' back onto 'é'
    }

    #[test]
    fn line_down_and_up_clamp() {
        let text = "one\ntwo\nthree";
        assert_eq!(line_down(text, 0, 1), 4); // start of 'two'
        assert_eq!(line_down(text, 0, 2), 8); // start of 'three'
