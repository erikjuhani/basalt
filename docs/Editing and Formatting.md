Basalt renders markdown by replacing syntax characters with visual styling. For example, heading `#` symbols are hidden and replaced with colored indicators, and block quotes display with a vertical bar instead of `>` characters.
...
See [[Links]] for more details on wiki-link behavior.
...
By default, Basalt opens notes in read-only mode. To edit notes:
...
- [[Known Limitations]] - Features not yet supported
- [[Configuration]] - Custom key bindings and commands
